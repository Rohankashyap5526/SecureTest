...
async function api(url,options={}){const r=await fetch(url,{credentials:'same-origin',...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});let d={};try{d=await r.json()}catch{}if(!r.ok)throw Error(d.error||'Request failed');return d}
...
async function login(){try{await api('/api/admin/login',{method:'POST',body:JSON.stringify({username:$('au').value,password:$('ap').value})});const session=await api('/api/admin/session');if(!session.authenticated)throw Error('Admin session could not be established. Check Render cookie/proxy settings.');navigate('/admin');await adminData()}catch(e){toast(e.message)}}
