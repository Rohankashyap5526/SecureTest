...
async function login(){const username=$('au').value.trim(),password=$('ap').value;if(!username||!password)return toast('Enter username and password.');const button=document.querySelector('#adminLogin .login-card .button');if(button){button.disabled=true;button.textContent='Signing in…'}try{await api('/api/admin/login',{method:'POST',body:JSON.stringify({username,password})});const session=await api('/api/admin/session');if(!session.authenticated)throw Error('Admin session was not created. Check Render session configuration.');navigate('/admin')}catch(e){toast(e.message)}finally{if(button){button.disabled=false;button.innerHTML='Sign in to dashboard <span>→</span>'}}}
...
function filterResults(){const input=$('resultSearch');if(!input)return;const q=input.value.toLowerCase().trim();const rows=allResults.filter(r=>Object.values(r).some(v=>String(v??'').toLowerCase().includes(q)));$('resultCount').textContent=`${rows.length} result${rows.length===1?'':'s'}`;$('results').innerHTML=rows.length?rows.map(r=>`<tr><td><div class="candidate-cell"><strong>${esc(r.Full_Name)}</strong><small>${esc(r.Mobile_Number)}</small></div></td><td>${esc(r.College_Name)}</td><td><span class="roll-badge">${esc(r.Roll_Number||"—")}</span></td><td>${esc(r.Mobile_Number)}</td><td>${esc(r.Course)} · ${esc(r.Branch)} · ${esc(r.Semester)}</td><td>${esc(r.Test_Name)}</td><td><span class="score">${r.Marks}/${r.Total_Marks}</span></td><td><span class="percent">${r.Percentage}%</span></td></tr>`).join(''):'<tr><td colspan="8" style="text-align:center;padding:28px;color:var(--subtle)">No matching results.</td></tr>'}
